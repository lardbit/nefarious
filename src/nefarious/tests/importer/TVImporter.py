                    ...
                    if not title:
                        if self._ingest_depth(file_path) > 0:
                            ...
                            for parent_title in parent_titles:
                                ...
                                logging.warning('[NO_MATCH_TITLE] Could not match nested file "{}"'.format(file_path))
                                return
                        else:
                        ...
                        if parser.is_single_episode():
                            ...
                            for result in results['results']:
                                ...
                                if parser.normalize_media_title(result['name']) == title:
                                    ...
                                    logging.info('[MATCH] Saved episode "{}" from file "{}"'.format(watch_episode, file_path))
                                    break
                            else:  # for/else
                                logging.warning('[NO_MATCH_MEDIA] No media match for title "{}" and file "{}"'.format(title, file_path))
                        else:
                            logging.warning('[NO_MATCH_SINGLE_EPISODE] No single episode title match for title "{}" and file "{}"'.format(title, file_path))
                    else:
